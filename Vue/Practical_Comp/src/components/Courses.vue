<template lang='html'>
  <form v-cloak>
      <div class='bar'>
          ...
          <input type='text' v-model='searchString' placeholder='Search courses...' />
      </div>
      ...
      <ul>
          ...
          <li v-for='course in courses | searchFor searchString'>
              <a v-bind:href='course.url'><img v-bind:src='course.image' /></a>
              <p>{{ course.title }}</p>
          </li>
      </ul>
  </form>
...
<script>
import Vue from 'vue'
Vue.filter('searchFor', (value, searchString) => {
  ...
  const userSearch = searchString.trim().toLowerCase()

  result = value.filter((course) => (course.title.toLowerCase().indexOf(userSearch) !== -1))
